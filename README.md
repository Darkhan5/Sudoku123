...
# Опционально: Supabase для рейтинга и PvP-комнат на Vercel
SUPABASE_URL=...
...
Для Vercel добавь все переменные окружения из `.env.local` в Project Settings -> Environment Variables. Секреты должны быть только без префикса `NEXT_PUBLIC_`, кроме `NEXT_PUBLIC_SITE_URL`.

PvP-комнаты и рейтинг локально пишутся в `.data`, но на Vercel файловая система не подходит для постоянного состояния. Для нормальной работы PvP на деплое подключи Supabase и создай таблицы:
