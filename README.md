...
- Городской и казахстанский рейтинг.
- Набор тестов для экономики, подписки, Stripe fulfillment, PvP-логики, рейтинга, онбординга, орнаментов, серии дней и подсказок.

## Рейтинг и очки

Рейтинг строится отдельно на каждый день по дате Казахстана. Вкладка `Город` показывает игроков из города текущего игрока, вкладка `Казахстан` показывает всех игроков страны. У одного игрока за день хранится только лучший результат.

Места занимают по очкам: чем больше очков, тем выше место. Формула результата:

```text
очки = 10 000 - время в секундах x 4 - ошибки x 360 - подсказки x 180
```

Минимум за решённую задачу — 1 000 очков. Если очки равны, выше становится игрок с меньшим временем, затем с меньшим числом ошибок, затем с меньшим числом подсказок, затем тот, кто отправил результат раньше.
