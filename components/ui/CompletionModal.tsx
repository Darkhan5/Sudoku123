...
export function CompletionModal({
  ...
  function shareOrnamentResult() {
    const text = finalOrnament
      ? `Режим Өрнек — Қазақ Судоку. Партия завершена орнаментом ${finalOrnament.kazakhName}: ${finalOrnament.meaning}`
      : "Режим Өрнек — Қазақ Судоку.";
    if (navigator.share) {
      void navigator.share({ title: "Қазақ Судоку", text });
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-40 grid place-items-center bg-slate-950/60 px-4 backdrop-blur-sm">
      <section className="w-full max-w-[420px] rounded-2xl bg-white p-5 text-center shadow-2xl" role="dialog" aria-modal="true">
        <div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-cyan-100 text-3xl">✓</div>
        <h2 className="mt-3 text-2xl font-bold text-slate-950">Решено</h2>
        <p className="mt-1 text-sm text-slate-500">Результат готов для общего рейтинга.</p>
        ...
        <div className="mt-5 rounded-xl border border-cyan-100 bg-cyan-50 p-4">
          ...
          <p className="mt-1 text-2xl font-bold text-slate-950">+{reward} алмазов</p>
          {iconReward ? (
        ...
        <p className="mt-4 text-sm font-semibold text-slate-700">
          {rank ? `Место в общем рейтинге: #${rank}` : "Результат отправлен в общий рейтинг"}
        </p>
